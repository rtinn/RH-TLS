...
*/
    ...
    public function GetLeaveTodayE($id) {
        $sql = "SELECT `emp_leave`.*, 
                       `employee`.`em_id`, `first_name`, `last_name`, 
                       `notif`.`id_conge`, `st_n`, `st_emp` , `st_admin`
                FROM `emp_leave`
                LEFT JOIN `employee` ON `emp_leave`.`em_id` = `employee`.`em_id`
                LEFT JOIN `notif` ON `emp_leave`.`id_conge` = `notif`.`id_conge`
                WHERE `emp_leave`.`em_id` = '$id'";
        $query = $this->db->query($sql);
        return $query->result();
    }
    ...
    public function updateStEmpTo2($id_conge) {
        $this->db->set('st_emp', '2');
        $this->db->where('id_conge', $id_conge);
        $this->db->update('notif');
    }
    

    public function updateStAdminTo2($id_conge) {
        $this->db->set('st_admin', '2');
        $this->db->where('id_conge', $id_conge);
        $this->db->update('notif');
    }
    ...
    public function GetNallApplication($emid){
        $sql = "SELECT `emp_leave`.*,
