...
$(document).ready(function() {
                    ...
                    <?php if($this->session->userdata('user_type')== 'EMPLOYEE'){ ?>
                        if (notification.st_emp === '1') {
                            notificationsHtml += '<a href="javascript:updateStEmp(' + notification.id_conge + ')">' +
                                ...
                                '<span class="mail-desc">mais en attente d\'approbation par RH</span>' + 
                                '<span class="time">' + notification.apply_date + '</span>' +
                                '</div>' +
                                '</a>';
                        } if (notification.st_admin === '1') {
                            notificationsHtml += '<a href="javascript:updateStAdmin(' + notification.id_conge + ')">' +
                                '<div class="btn btn-danger btn-circle"><i class="fa fa-link"></i></div>' +
                                '<div class="mail-contnet">' +
                                '<span class="mail-desc">Demande congé ' + notification.typeid + '</span>'+
                                '<span class="mail-desc">' + notification.leave_status + ' par RH</span>'+
                                '<span class="time">' + notification.apply_date + '</span>' +
                                '</div>' +
                                '</a>';
                        }
    ...
    // Fonction pour mettre à jour st_emp et rediriger
    window.updateStEmp = function(id_conge) {
        $.ajax({
            url: url + 'leave/updateStEmp',
            type: 'POST',
            data: {id_conge: id_conge},
            success: function(response) {
                // Recharger les notifications après la mise à jour (optionnel)
                loadNotifications();

                // Rediriger vers la page leave/Application après la mise à jour
                window.location.href = url + 'leave/Application';
            }
        });
    };

    // Fonction pour mettre à jour st_admin et rediriger
    window.updateStAdmin = function(id_conge) {
        $.ajax({
            url: url + 'leave/updateStAdmin',
            type: 'POST',
            data: {id_conge: id_conge},
            success: function(response) {
                // Recharger les notifications après la mise à jour (optionnel)
